...
from model20.custom_layers import EqualizedLinear, Conv_Block, ScaleBlock, PixelNormalizationLayer
from model20.dataset import VariableImageFolder
from model20.utility import MinibatchSTD, compute_gradient_penalty, exp_avg_update, calc_padding
...
class ProGAN(pl.LightningModule):
    ...
    def add_depth(self):
        ...
        logging.debug("Adding Depth, Scale:", scale + 1, "Img Size:", self.img_size * 2)
    ...
    def load_model(self, path,  map_location=None, image_size=1024):
        self.image_size = image_size # to check for changes in second model generate call
        if map_location is None:
             map_location= torch.device("cuda" if torch.cuda.is_available() else "cpu")
        checkpoint = torch.load(path, map_location=map_location)
        for i, _ in enumerate(range(checkpoint['settings']['g_depth'])):
            current_dim_scale = 2**(i+3)
            logging.debug(f"{current_dim_scale}") # compute current scale
            self.add_depth()
            if current_dim_scale >= image_size:
                break # break once desired scale is reached.
        self.load_state_dict(checkpoint['state_dict'], strict=False)
        self.set_settings(checkpoint['settings'])
        logging.debug("Model & Settings Successfully Loaded")
        return self
